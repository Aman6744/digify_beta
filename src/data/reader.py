...
class Dataset():
    ...
    def read_partitions(self):
        ...
        for y in self.partitions:
            self.dataset[y]['image'] += dataset[y]['image']
            self.dataset[y]['label'] += dataset[y]['label']

    def save_partitions(self, target, image_input_size, maxTextLength):
        ...
        with h5py.File(target, "w") as hf:
            for pt in self.partitions:
                ...
                for i in reversed(range(len(self.dataset[pt]["image"]))):
                    transcription = pp.preprocess_label(self.dataset[pt]["label"][i], maxTextLength)
                    if not transcription[0]:
                        self.dataset[pt]['image'].pop(i)
                        self.dataset[pt]['label'].pop(i)
                size = (len(self.dataset[pt]['image']),) + image_input_size[:2]
                total += size[0]

                dummy_image = np.zeros(size, dtype=np.uint8)
                dummy_sentence = [("c" * maxTextLength).encode()] * size[0]

                hf.create_dataset(f"{pt}/image", data=dummy_image, compression="gzip", compression_opts=9)
                hf.create_dataset(f"{pt}/label", data=dummy_sentence, compression="gzip", compression_opts=9)

        pbar = tqdm(total=total)
        batch_size = 1024

        for pt in self.partitions:
            for batch in range(0, len(self.dataset[pt]['image']), batch_size):
                images = []

                with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
                    r = pool.map(partial(pp.preprocess_image, target_size=image_input_size, augmentation=self.dataset[pt]["augmentation"]), self.dataset[pt]['image'][batch:batch + batch_size])
                    images.append(r)
                    pool.close()
                    pool.join()

                with h5py.File(target, "a") as hf:
                    hf[f"{pt}/image"][batch:batch + batch_size] = images
                    hf[f"{pt}/label"][batch:batch + batch_size] = [s.encode() for s in self.dataset[pt]['label'][batch:batch + batch_size]]
    ...
    def _init_dataset(self):
        ...
        for i in self.partitions:
            if i in ['test', 'valid']:
                dataset[i] = {"image": [], "label": [], "augmentation": False}
            else:
                dataset[i] = {"image": [], "label": [], "augmentation": True}
        ...
        for i in self.partitions:
            for line in paths[i]:
                try:
                    ...
                    dataset[i]['label'].append(gt_dict[line])
                    dataset[i]['image'].append(img_path)
                except KeyError:
                    pass
