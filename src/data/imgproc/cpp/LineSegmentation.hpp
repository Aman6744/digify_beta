...
class Region {
    ...
    private:
        ...
        bool updateRegion(Mat &img1, Mat &img2, int);
        void calculateMean();
...
class LineSegmentation {
    public:
        ...
        Mat binaryImg, smoothedImg;
        vector<Rect> contours;
        Mat contoursDrawing;
        Mat linesDrawing;

        void segment(Mat input, Mat &_input, vector<Mat> &output, int chunksNumber, int chunksProcess);
        void getContours();
        void generateChunks();
