...
void Binarization::binarize(Mat image, Mat &output, Mat &_output, bool light, int option){
	Mat grayscale;
    ...
    thresholdImg(grayscale, output, _output, option, winx, winy, 0.1, 128);
}

void Binarization::thresholdImg(Mat im, Mat &output, Mat &smoothedImg, int option, int winx, int winy, double k, double dR){
	...
	if (option > 1){
		...
		threshold(smoothedImg, output, threshold_value, 255, THRESH_BINARY | THRESH_OTSU);
