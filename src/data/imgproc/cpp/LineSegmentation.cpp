...
void LineSegmentation::segment(Mat input, Mat &_input, vector<Mat> &output, int chunksNumber, int chunksProcess) {
    this->binaryImg = input.clone();
    this->smoothedImg = _input.clone();
...
void LineSegmentation::generateRegions() {
    ...
    r->updateRegion(this->binaryImg, this->smoothedImg, 0);
    ...
    for (int i=0; i<this->initialLines.size(); ++i) {
        ...
        bool res = r->updateRegion(this->binaryImg, this->smoothedImg, i);
...
bool Region::updateRegion(Mat &binaryImg, Mat &smoothedImg, int regionID) {
    ...
    bool ret = countNonZero(region) == region.cols * region.rows;
    
    minRegionRow = rowOffset = (top == nullptr) ? 0 : top->minRowPosition;
    maxRegionRow = (bottom == nullptr) ? smoothedImg.rows : bottom->maxRowPosition;

    start = min(minRegionRow, maxRegionRow), end = max(minRegionRow, maxRegionRow);
    region = Mat::ones(end - start, smoothedImg.cols, CV_8U) * 255;

    for (int c=0; c<smoothedImg.cols; c++) {
        int start = ((top == nullptr) ? 0 : top->points[c].x);
        int end = ((bottom == nullptr) ? smoothedImg.rows - 1 : bottom->points[c].x);

        if (end > start)
            this->height = max(this->height, end - start);

        for (int i=start; i<end; i++)
            region.at<uchar>(i - minRegionRow, c) = smoothedImg.at<uchar>(i, c);
    }
    return ret;
}
